...
// Main execution
const holidaysYamlPath = path.join(
  __dirname,
  "../holiday_jp/holidays_detailed.yml"
);
const holidays = yaml.load(fs.readFileSync(holidaysYamlPath, "utf8")) as Record<
  string,
  any
>;
...
const holidays_every_year = Object.keys(holidays).reduce(
  (holidays_every_year: Record<string, Record<string, any>>, date) => {
    const year = moment(new Date(date)).format("YYYY");
    const holidays_per_year = {
      ...holidays_every_year[year],
      [date]: holidays[date],
    };
...
for (const year of Object.keys(holidays_every_year) as Array<
  keyof typeof holidays_every_year
>) {
